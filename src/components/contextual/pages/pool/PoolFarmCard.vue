...
const rewardAPR = computed(() => {
  console.log('CALCULATING POOL APR');
    ...
    const sharePrice = Number(props.pool?.totalLiquidity) / Number(props.pool?.totalShares)
    const tvl = sharePrice * props.farm.pools[0].slpBalance * 0.000000000000000001
    ...
    const symmPerSecond = ((props.farm.pools[0].allocPoint / props.farm.pools[0].symmChef.totalAllocPoint) * props.farm.pools[0].symmChef.symmPerSecond) / 1e18
    const symmPerBlock = symmPerSecond * averageBlockTime
    const symmPerDay = symmPerBlock * blocksPerDay
    const rewardAprPerYearToken1 =
      (Number(toFiat(1, getAddress(props.farm.pools[0].symmChef.symm))) * symmPerDay * 365) / tvl * 100;
      console.log("rewardAprPerYearToken1", rewardAprPerYearToken1)

      let rewardAprPerYearToken2 = 0;
    if (props.farm.pools[0].rewarder !== null) {
      const rPerSecond =
            props.farm.pools[0].rewarder.id === '0xed803f7035749de7ae59b4e70af54111ec61d256'
              ? (((props.farm.pools[0].rewarderAllocPoint - 100) / props.farm.pools[0].rewarder.totalAllocPoint) * props.farm.pools[0].rewarder.rewardPerSecond) /
                1e18
              : ((props.farm.pools[0].rewarderAllocPoint / props.farm.pools[0].rewarder.totalAllocPoint) * props.farm.pools[0].rewarder.rewardPerSecond) / 1e18
          const rPerBlock = rPerSecond * averageBlockTime
          const rPerDay = rPerBlock * blocksPerDay

      rewardAprPerYearToken2 = 
        (Number(toFiat(1, getAddress(props.farm.pools[0].rewarder.rewardToken))) * Number(rPerDay) * 365) / tvl  * 100  
        console.log("rewardAprPerYearToken2", rewardAprPerYearToken2)
    }
    const roiPerYear = rewardAprPerYearToken1 + rewardAprPerYearToken2 + feeApyPerYear
...
const rewardTokens  = computed(():
 ...
 {rewardsPerDay: string,
  ...
  let rTokens: { [address: string]: {rewardsPerDay: string, tokenAddress: string} } = {};
  rTokens[token1Address] = {rewardsPerDay:symmPerDay.value, tokenAddress: token1Address}
  if (props.farm.pools[0].rewarder !== null) {
    const rPerSecond =
            props.farm.pools[0].rewarder.id === '0xed803f7035749de7ae59b4e70af54111ec61d256'
              ? (((props.farm.pools[0].rewarderAllocPoint - 100) / props.farm.pools[0].rewarder.totalAllocPoint) * props.farm.pools[0].rewarder.rewardPerSecond) /
          ...
          const rPerBlock = rPerSecond * averageBlockTime
          const rPerDay = rPerBlock * blocksPerDay
    rTokens[getAddress(props.farm.pools[0].rewarder.rewardToken)] = {rewardsPerDay:rPerDay.toFixed(4), tokenAddress: getAddress(props.farm.pools[0].rewarder.rewardToken)}
